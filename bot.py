...
class AIAssistant:
    def get_ai_response(self, user_message, user_context=""):
        ...
        # Быстрые предопределенные ответы
        responses = {
            "расписание": "📅 Чтобы узнать расписание, используйте кнопку '📅 Ближайшие занятия'. Там вы найдете актуальное расписание вашей группы! 🎓",
            "оплата": "💳 Информацию о балансе и оплате можно найти в разделе '💳 Баланс и оплата'. Там же есть ссылка для пополнения счета! 💰",
            "занятия": "🎯 Наши занятия проходят в небольших группах с опытными преподавателями. Подробнее о вашей группе - в личном кабинете! ✨",
            "преподаватели": "👨‍🏫 У нас работают квалифицированные преподаватели с большим опытом. Каждый из них специализируется на своей предметной области! 🎓",
            "стоимость": "💰 Стоимость занятий зависит от программы и группы. Точную информацию о вашем балансе можно узнать в разделе '💳 Баланс и оплата' 💫",
            "контакты": "🌐 Наша группа ВКонтакте: vk.com/vdvascheta37 📞 Для связи с администрацией используйте кнопку '🌐 ВКонтакте' 📧",
            "программы": "📚 У нас есть программы по математике, русскому языку, подготовке к школе и развитию логики! Подробности - у администратора 🎯"
        }
        
        # Ищем ключевые слова в вопросе
        user_lower = user_message.lower()
        
        if any(word in user_lower for word in ['расписан', 'время', 'когда', 'заняти', 'день', 'недел']):
            return responses["расписание"]
        elif any(word in user_lower for word in ['оплат', 'баланс', 'деньг', 'стоимос', 'цена', 'плат', 'денег']):
            return responses["оплата"]
        elif any(word in user_lower for word in ['заняти', 'урок', 'учёб', 'обучен', 'курс']):
            return responses["занятия"]
        elif any(word in user_lower for word in ['преподавател', 'учител', 'педагог', 'тренер']):
            return responses["преподаватели"]
        elif any(word in user_lower for word in ['стоимос', 'цен', 'деньг', 'плат', 'денег', 'рубл']):
            return responses["стоимость"]
        elif any(word in user_lower for word in ['контакт', 'телефон', 'адрес', 'связ', 'написат', 'звонит']):
            return responses["контакты"]
        elif any(word in user_lower for word in ['программ', 'предмет', 'математ', 'русск', 'лог', 'развити']):
            return responses["программы"]
        else:
            return "🤖 Я помогу вам с информацией о расписании, оплате и занятиях! Используйте кнопки меню для быстрого доступа к нужному разделу. Если нужна конкретная информация - спросите о расписании, оплате или преподавателях! 🎓✨"
